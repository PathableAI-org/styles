...
- [x] T015 Create `packages/styles/src/pathable-component-wrappers/pathable-segmented-control.scss` with a border-box `.pathable-segmented-control` track using tokenized gap, radius, background, and padding. Constrain horizontal controls to `max-width: 100%` with internal scrolling, expose radius/gap/focus-ring custom properties, and provide vertical and static presentation variants.
- [x] T016 [US3] Add a border-box `.pathable-segmented-control__option` using tokenized gap/padding and a nonnegative calculated radius. Apply shared hover, focus, and disabled states; keep disabled text readable; and style `.pathable-segmented-control__option--selected`, `aria-checked="true"`, and `aria-pressed="true"` equivalently with persistent background, border, and weight signals (FR-015).
...
- [x] T019 [P] [US3] Create executable Storybook reference fixtures at `packages/styles/src/stories/interaction-controls/SegmentedControl.stories.ts` covering single-select, multi-select, vertical, disabled, static single-option, constrained overflow, selected interaction states, and focus surfaces with deterministic `play` assertions.
