...
- **React Modal**: Single React component that applies styles-owned open/closed presentations, emits the documented dual-class shell when open, and owns transitions plus focus/scroll/keyboard behavior. Exposes `closeOnBackdropClick` (boolean, default `false`) for optional overlay-click dismiss; ARIA/`...rest` stay on the dialog element.
