...
> components require automatic default-token fallback styling. React now imports
> the theme, component, and utility layers; the styles subpaths remain independently
> importable for CSS-only consumers.
...
| 8   | `@pathableai/react` no longer imports the default theme tokens via its side-effect import                                                | Superseded | `pnpm test:next-consumer` now verifies the automatic fallback import and emitted consumer CSS; see the current contract update above                                                                                                         |
| 9   | The `--pathable-color-*` declarations are consolidated into one low-specificity root block                                               | Satisfied  | `pnpm lint:tokens`; `specs/057-package-exports/contracts/package-exports.md`                                                                                                                                                                 |
...
pnpm test:next-consumer                       # Structural subpaths + automatic React fallback
