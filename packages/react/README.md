...
| Modal         | React-owned portaled overlay with background isolation, focus trapping, backdrop/Escape close, scroll locking, and focus restoration. | `open`, `onClose`, `title`, `description`, `children`, `footer`, `closeLabel`, `initialFocusRef` |
...
`Modal` renders the complete overlay hierarchy into `document.body`. While open, it marks background body siblings `inert` and `aria-hidden`, then restores their prior values when closed. React owns its lifecycle, so consumers do not need `@pathableai/styles/js` for modal behavior; if the global styles runtime is already present, it ignores the React-owned modal.
