...
| TASK-016 | Split version-PR creation from publication so only the publish job uses the protected `npm` environment, npm 11.19.0, and `id-token: write`; both paths install frozen dependencies under Node 24.                                                                                                  | ✅        | 2026-08-12 |
| TASK-017 | Run the packed Next.js consumer smoke test in pull-request CI so the reviewed tarballs, React runtime externalization, public CSS contract, and referenced assets are validated before release handling.                                                                                            | ✅        | 2026-08-12 |
...
| TASK-019 | Configure Changesets Action to run `pnpm release` after the version PR is merged, create GitHub releases and tags, and authenticate to npm through OIDC. Assign only the publish job to the protected `npm` environment with `contents: write` and `id-token: write`; do not set `NODE_AUTH_TOKEN`. | ✅        | 2026-08-12 |
